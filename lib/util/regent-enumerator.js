...
class RegentEnumerator extends BaseObject
{
    ...
    constructor(aliases = [], overrides = {})
    {
        ...
        _private(this).byIndex = new Map();

        /** @protected {Map} A map of aliased values */
        _private(this).byAlias = new Map();

        Object.keys(overrides).forEach((alias) => {
            _private(this).byIndex.set(overrides[alias], alias);
            _private(this).byAlias.set(alias, overrides[alias]);
        });

        let index = 1;
        for (let i = 0, li = aliases.length; i < li; ++i) {
            while (_private(this).byIndex.get(index)) {
                index <<= 1;
            }
            _private(this).byIndex.set(index, aliases[i]);
            _private(this).byAlias.set(aliases[i], index);
        }
    }
    ...
    getValue(alias)
    {
        return _private(this).byAlias.get(alias) || null;
    }
    ...
    getAlias(index)
    {
        return _private(this).byIndex.get(index) || null;
    }
}
