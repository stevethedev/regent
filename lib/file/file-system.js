...
class FileSystem extends RegentObject {
    ...
    /**
     * Find the absolute path to a given filename
     *
     * @method getPath
     *
     ...
     */
    getPath(fileName = '') {
        return this.call(getNormalizedPath, fileName);
    }


    /**
     * Find the mime-type from a file name
     *
     * @method getMime
     *
     * @param {String} fileName
     ...
     */
    getMime(fileName) {
        return FileMime.create().getType(fileName);
    }
 ...
 *
 * @return {Integer} The amount of data transferred
 */
...
function sanitize(fileName) {
    ...
    return fileName.replace(path.parse(path.normalize(fileName)).root, '');
}
