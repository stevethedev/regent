...
class QueryBuilder extends BaseQueryBuilder
{
    ...
    decrement(field, value = 1, values = {})
    {
        ...
        decrementMap.forEach((value, field) => {
            if (!valueMap.has(field)) {
                fields.push(dialect.decrement(field, value, binding));
            }
        });

        valueMap.forEach((value, field) => {
            fields.push(dialect.assign(field, value, binding));
        });

        return this.updateRaw(fields.join(', '), binding);
    }
    ...
    increment(field, value = 1, values = {})
    {
        ...
        incrementMap.forEach((value, field) => {
            if (!valueMap.has(field)) {
                fields.push(dialect.increment(field, value, binding));
            }
        });
