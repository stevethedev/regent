...
class QueryBuilder extends BaseQueryBuilder
{
    constructor(connection, settings = {})
    {
        ...
        /** @protected */
        self.clauses = {
            from:   self.settings.clauses.FromClause.create(self.settings),
            select: self.settings.clauses.SelectClause.create(self.settings),
        };
    }
    ...
    {
        table = this.call(applyDialect, 'table', table);
        if (alias) {
            alias = this.call(applyDialect, 'alias', alias);
        ...
        fields.forEach((field) => {
            if (field && 'object' === typeof field) {
                return Object.keys(field).forEach((alias) => {
                    const fieldName = this.call(applyDialect, 'field', field[alias]);
                    const aliasName = this.call(applyDialect, 'alias', alias);
                    signature.push(`${fieldName} AS ${aliasName}`);
                });
            }
            field = this.call(applyDialect, 'field', field);
...
/**
 * Apply a dialect field to the value
 *
 * @private
 * @method applyDialect
 *
 ...
 * @return {String}
 */
function applyDialect(name, value)
{
    const dialect = _protected(this).settings.dialect;
    if(dialect[name]) {
        return dialect[name].replace('$1', value);
    }
    return value;
}
