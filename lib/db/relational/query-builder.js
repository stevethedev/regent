...
class QueryBuilder extends BaseQueryBuilder {
    ...
    whereNotBetween(field, range, inclusive = true) {
        ...
        return this.whereRaw(`(${left} OR ${right})`, bound);
    }
 ...
 * @return {this}
 */
function whereInOperator(field, values, { include }) {
    const { dialect } = $protected(this);
    const bound       = [];
    const valueSet    = RegentSet.create(values).toArray();

    return this.whereRaw(
        dialect.whereIn(bound, field, valueSet, { include }),
        bound
    );
}
