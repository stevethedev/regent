...
class QueryBuilder extends BaseQueryBuilder {
    ...
    whereIn(field, values) {
        const bound = [];
        const signature = $protected(this).dialect.whereIn(
            bound,
            field,
            RegentSet.create(values).toArray()
        );
        return this.whereRaw(signature, bound);
    }
    ...
    whereNotIn(field, values) {
        const bound = [];
        const signature = $protected(this).dialect.whereNotIn(
            bound,
            field,
            RegentSet.create(values).toArray()
        );
