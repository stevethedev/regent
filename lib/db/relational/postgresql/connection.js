...
const QueryBuilder    = require(
    'regent-js/lib/db/relational/postgresql/query-builder'
);
...
const BASE_CONFIG     = require(
    'regent-js/lib/db/relational/postgresql/config'
);
...
class PostgresDb extends RelationalDb {
     ...
     */
    getDialect() {
        return PostgresDialect.create({ prefix: this.getPrefix() });
    }
}
...
async function sendQuery(query, bound) {
    ...
    const { rows, rowCount } = await pool.query(query, bound);
    return new RecordCollection(rows, rowCount);
}
