...
class BaseDialect extends BaseObject {
    ...
    where(bound, whereObjects) {
        ...
        return whereObjects.map((whereObject, index) => {
            ...
            const signature  = (whereObject.query)
                ...
                : this.call(whereBase,  bound, whereObject);
     ...
     * @return {String}
     */
    sqlSelect(bound, { columns, distinct, limit, offset, table, where }) {
        if (!columns.length) {
            columns = '*';
        }
...
/**
 * Compile a normal SQL WHERE signature
 *
 * @private
 * @method whereBase
 *
 * @param {Mixed[]} bound
 * @param {Mixed[]} values
 * @param {String}  whereObject.signature
 * @param {String}  whereObject.values
 *
 * @return {String}
 */
function whereBase(bound, { signature, values }) {
