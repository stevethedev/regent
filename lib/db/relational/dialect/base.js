...
class BaseDialect extends BaseObject {
    ...
    where(bound, whereObjects) {
        ...
        return whereObjects.map(({ joiner, signature, values }, index) => {
            signature = signature.replace(/{(\d+)}/g, (match, valueId) => {
                if (Number(valueId) > values.length) {
                    return match;
                }
                return this.value(bound, values[valueId]);
            });
            if (0 < index) {
                signature = `${joiner || 'AND'} ${signature}`;
            }
            return signature;
        }).join(' ');
    }
