...
class Logger extends RegentObject
{
    ...
    log(...text)
    {
        if (_private(this).config.logLevel >= LOG_LEVEL_NORMAL) {
            this.call(writer, text, COLOR_LOG);
        }
        return this;
    }
    ...
    info(...text)
    {
        if (_private(this).config.logLevel >= LOG_LEVEL_INFO) {
            this.call(writer, text, COLOR_INFO);
        }
        return this;
    }
    ...
    warn(...text)
    {
        if (_private(this).config.logLevel >= LOG_LEVEL_WARN) {
            this.call(writer, text, COLOR_WARN);
        }
        return this;
    }
    ...
    error(...text)
    {
        if (_private(this).config.logLevel >= LOG_LEVEL_ERROR) {
            this.call(writer, text, COLOR_ERROR);
        }
        return this;
    }
}
 ...
 */
function writer(text, color = '')
{
    const date      = DateFormat('yyyy-mm-dd');
    const fileName  = `${_private(this).logDirectory}/regent-${date}.log`;
    const timestamp = `[${DateFormat('isoDateTime')}]`;
    const message   = `${text.join(' ')}`;
    const resets    = `${COLOR_RESET}${NEWLINE}`;
    if (!FileSystem.existsSync(_private(this).logDirectory)) {
        FileSystem.mkdirSync(_private(this).logDirectory);
    }
    process.stdout.write(`${color}${timestamp} ${message}${resets}`);
    FileSystem.appendFileSync(fileName, `${timestamp} ${message}${NEWLINE}`);
}
