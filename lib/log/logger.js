...
class Logger extends RegentObject {
    constructor(regent, config, logDirectory) {
        ...
        $private(this).logDirectory = logDirectory;
        $private(this).fileSystem   = new FileSystem(regent, logDirectory);
    }
...
async function writer(prefix, text, color = '') {
    ...
    if (!await fileSystem.dirExists($private(this).logDirectory)) {
        // Create folder
        await fileSystem.createDir($private(this).logDirectory);
    }
