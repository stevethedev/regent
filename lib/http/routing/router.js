...
const RegentRouter      = require('regent/lib/core/routing/router');
const HttpError404      = require('regent/lib/http/errors/http-error-404');
const HttpRoute         = require('regent/lib/http/routing/route');
const HttpRouteBinder   = require('regent/lib/http/controllers/route-binder');
const HttpRouteCompiler = require(
    'regent/lib/http/routing/compiler/route-compiler'
);
const HttpController    = require('regent/lib/http/controllers/abstract');
...
class HttpRouter extends RegentRouter {
    ...
    resource(uri, controller, options = {}) {
        if ('string' === typeof controller) {
            controller = Directories.requireApp(controller);
        }
        assert.isString(uri);
