...
const HttpController    = requireLib('http/controller/abstract');
...
class HttpRouter extends RegentRouter
{
    ...
    {
        assert.isString(uri);
        assert.instanceOf(controller, HttpController);
        assert.isObject(options);

        _protected(this).routeBinder.bindController(controller, options);
    ...
    match(methods, uri, handler, options)
    {
        ...
        const that = _protected(this);
        const route = new HttpRoute(this.getRegent(), uri, handler, options);
