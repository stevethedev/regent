...
function compileSimpleRoute(route)
{
    ...
    return new SimpleCompiledHttpRoute(regent, handler, uri, caseSensitive);
}
...
function getVariablePattern(route, variableName)
{
    ...
    return route.getPattern(variableName)
...
function getRegexp(route)
{
    ...
    const pattern     = this.call(getSplitUri, route)
        .reduce((pattern, next) => {
            ...
            const varValue = this.call(getVariablePattern, route, varname);
            return `${pattern}(${varValue})${next}`;
        });
