...
const SimpleCompiledHttpRoute = require(`${R_COMPILER}/simple-compiled-route`);
...
class RouteCompiler extends RegentObject {
    ...
    constructor(regent) {
        ...
        const globalVariablePatterns = new RegentMap();
        $protected.set(this, { globalVariablePatterns });
    }
    ...
    setGlobalPattern(name, pattern) {
        if (name && 'object' === typeof name) {
            for (const [ key, value ] of Object.entries(name)) {
                this.setGlobalPattern(key, value);
            }
        } else {
            assert.isString(name);
            const { globalVariablePatterns } = $protected(this);

            if (null === pattern) {
                globalVariablePatterns.delete(name);
            } else {
                assert.isString(pattern);
                globalVariablePatterns.set(name, pattern);
            }
        }
        return this;
    }
    ...
    getGlobalPattern(name) {
        assert.isString(name);
        return $protected(this).globalVariablePatterns.getOr(name, null);
    }
...
function compileRegexpRoute(route) {
    ...
    const group         = this.call(getRouteType, route);
    const middleware    = this.call(getMiddleware, route);

    return new RegexpCompiledHttpRoute(regent, prefix, handler, {
        caseSensitive,
        group,
        middleware,
        regexp,
...
function compileSimpleRoute(route) {
    ...
    return new SimpleCompiledHttpRoute(regent, uri, handler, {
        caseSensitive,
        group,
        middleware,
    });
...
function getVariableSet(route, hideDecoration = true) {
    const variableSet  = new RegentSet();
...
function getRegexp(route) {
    ...
    const varIterator = variableSet.entries();
