...
const EventList      = require('regent/lib/event/event-list');
...
class CompiledHttpRoute extends RegentObject {
     ...
     * @inheritDoc
     */
    constructor(regent, routeType, handler, middleware) {
        ...
        that.kernel       = regent.getKernel(HTTP);
...
async function loadMiddleware(request) {
    const that = $protected(this);
    const { routeType, kernel } = that;

    const middlewareGroup = kernel.getMiddlewareGroup(routeType);
    const routeMiddleware = that.middleware;

    await request.addMiddleware(...middlewareGroup, ...routeMiddleware);
}
