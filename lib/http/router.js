...
const RegentRouter = requireLib('core/router');
 ...
 * within this system.
 */
class Router extends RegentRouter
{
    constructor(regent)
    {
        super(regent);
        ...
        HTTP_METHODS.forEach(method => {
            this.__routes.set(method, new Map());
        });
    ...
    getCallback(method, uri)
    {
        ...
        assert.isString(uri);
        return this.__routes.get(method).get(uri) || null;
    }
    ...
    match(methods, uri, callback)
    {
        ...
        methods.forEach(method => {
            ...
            method = method.toUpperCase();
            assert.include(HTTP_METHODS, method);
            this.__routes.get(method).set(uri, callback);
        });
