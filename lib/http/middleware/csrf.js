...
class CsrfMiddleware extends Middleware {
    ...
    isProtected(httpMethod) {
        return Boolean($protected(this).protect.get(httpMethod.toUpperCase()));
    }
    ...
    async run(request, response, next) {
        ...
        this.getRegent().getEmitter()
            .unique(HTTP_BEFORE_RENDER, addCsrfFunction);
        ...
        response.getCookie(CSRF_COOKIE)
            .setHttp(true)
            .setSameSite(true)
            .setValue(csrfToken);
        ...
        if (kernel.getConfig('useHttps')) {
            response.getCookie(CSRF_COOKIE).setSecure(true);
        }
 ...
 * @return {this}
 */
function addCsrfFunction(response, context) {
    // eslint-disable-next-line camelcase
    context.csrf_token = () => {
        return response.getSession().get(CSRF_FIELD);
    };
...
async function protectRequest(request, response) {
    ...
    if (request.hasCookie(CSRF_COOKIE)) {
        token = request.getCookie(CSRF_COOKIE).getValue();
    } else {
