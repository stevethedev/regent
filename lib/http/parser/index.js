...
class BodyParser extends BaseObject {
    ...
    constructor(contentType, body, fileSystem, encoding) {
        ...
        if (CONTENT_TYPE.test(contentType)) {
            contentType = contentType.replace(CONTENT_TYPE, '');
        }
    ...
    async getMultipart() {
        ...
        if (!cache.has(MULTIPART)) {
            cache.set(MULTIPART, await this.call(getMultipart, body));
        }
 ...
 * @return {Object}
 */
async function getMultipart(body) {
    const result = {};
...
async function getFile(fileName, body, fieldEnd) {
    ...
    const mimer    = new FileMime();
    const mimetype = mimer.getType(fileName);
    const ext      = mimer.getExtension(mimetype);
    const fakeName = `${await crypto.random(RANDOM_FILE_SIZE)}.${ext}`;
