        ...
        actions.forEach((action) => {
            ...
            if ('function' !== typeof controller.prototype[action.action]) {
                this.getRegent().getLogger().warn(
                    `${controller.name} has no method named ${action.action}.`,
                    'This may cause problems if a user attempts to connect to',
                    `HTTP ${action.method} ${uri}`
                );
            }

            router
                .match(methods, uri, handler, options)
                .route(`${name}.${action.action}`);
        });
