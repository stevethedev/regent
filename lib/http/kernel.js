...
class HttpKernel extends Kernel {
    ...
    constructor(regent, config = {}) {
        ...
        /**
         * TCP/HTTP server object that this kernel manipulates.
         *
         * @protected
         * @type {http.Server}
         */
        const server = (config.useHttps ? Https : Http).createServer(
            onRequest.bind(this)
        );
    ...
    /**
     * Get a configuration option from the kernel
     *
     * @method getConfig
     *
     * @param  {String}  option
     ...
     */
    getConfig(option) {
        return $private(this).config[option];
    }
}
...
async function onRequest(req, res) {
    ...
    regent.getLogger().info(`HTTP ${req.httpVersion} ${req.method} ${req.url}`);

    const emitter = regent.getEmitter();

    const body = await getBody(req);

    const request   = new Request(this, req, body);
     ...
     */
    const fileSystem = new FileSystem(regent, regent.getDir(PUB_DIR).resolve());
