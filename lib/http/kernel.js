...
class HttpKernel extends Kernel
{
    ...
    constructor(regent, config)
    {
        ...
        // emitter.on(Events.HTTP_AFTER_RESPNSE, afterResponse);
    }
...
async function onRequest(req, res)
{
    ...
    const mwHandler = this.call(getMiddlewareHandler, request, response);

    this.call(loadBaseMiddleware, mwHandler);
    ...
    emitter.emit(Events.HTTP_BEFORE_CONNECT, request, response);
...
async function onBeforeConnect(request, response)
{
    return await request.runMiddlewares();
}

/** @private */
function loadBaseMiddleware(middlewareHandler)
{
    const baseMiddleware = this.getMiddleware();
    middlewareHandler.add(...baseMiddleware);
}

/** @private */
function getMiddlewareHandler(request, response)
{
    const handler = new MiddlewareHandler(this.getRegent(), request, response);

    const coreMiddleware = this.getMiddleware();
