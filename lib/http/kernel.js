...
class HttpKernel extends Kernel
{
    ...
    start()
    {
        const promise = (resolve, reject) => {
            ...
            if (Cluster.isMaster && this.__config.cluster) {
                this.call(__setupClustering);
            } else {
 ...
 */
/** @private */
function __setupClustering()
{
    const numWorkers = this.__config.processes;
    const Logger = this.getRegent().getLogger();
    ...
    Cluster.on('online', (worker) => {
        this.__workers.set(worker.process.pid, worker);
        Logger.info(`Worker ${worker.process.pid} is online`);
    });

    Cluster.on('exit', (worker, code, signal) => {
        this.__workers.delete(worker.process.pid);
